...
philly_tracts %>%
  ...
  summarize(
    n_tracts = n(),
    no_2015_match = sum(is.na(med_inc_15M))
  )
```
...
philly_tracts %>%
  filter(med_incE < 40000, dist_mi > 0.5) %>%
  summarize(
    n_low_income_no_access = n(),
    reliability_breakdown = list(table(reliability))
  )

ggplot(philly_tracts) +
  geom_sf(aes(fill = med_incE, color = NA)) +
  geom_sf(data = philly_tracts %>% filter(med_incE < 40000, dist_mi > 0.5), fill = "red", alpha = 0.5) +
  labs(title = "Low-Income Tracts Without Access to Indego Stations",
       subtitle = "Defined as median income < $40,000 and distance > 0.5 miles",
       fill = "Median Income") +
  theme_minimal()
